    ...
    (async () => {
      ...
      switch (msg.kind) {
        case MessageKind.ScanRequest: {
          const data = msg;
          console.info("[PromptMonitor:SW] ScanRequest", {
            id: data.id,
            url: data.url,
            method: data.method,
          });
          ...
          if (isFinal && emailsFound.some((e) => !isDismissed(e, dismissed))) {
            ...
            try {
              console.info("[PromptMonitor:SW] opening popup window", {
                url,
                width: POPUP_WIDTH,
                height: POPUP_HEIGHT,
              });
              ...
              console.error(
                "[PromptMonitor:SW] Failed to open popup window",
                err,
              );
            }
          }
        ...
        case MessageKind.DismissEmail: {
          console.info("[PromptMonitor:SW] DismissEmail", msg.email);
